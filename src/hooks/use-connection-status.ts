  ...
  const checkConnectionStatus = useCallback(async (connectionId: string): Promise<ConnectionStatus | null> => {
    try {
      ...
      return {
        ...
        provider: result.provider as 'evolution' | 'wapi' | 'uazapi' | 'meta',
        error: result.error,
      };
    } catch (error) {
      ...
      const updatedConnections = await Promise.all(
        conns.map(async (conn) => {
          ...
          return {
            ...
            provider: (status?.provider as 'evolution' | 'wapi' | 'uazapi' | 'meta' | undefined) || conn.provider,
            error: status?.error,
          };
        })
      );
      ...
      setConnections(
        updatedConnections.map((c) => ({
          ...
          phoneNumber: c.phoneNumber,
            provider: c.provider as 'evolution' | 'wapi' | 'uazapi' | 'meta',
          error: c.error,
        }))
      );
