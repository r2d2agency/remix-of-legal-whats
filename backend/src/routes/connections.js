...
router.patch('/:id', async (req, res) => {
  try {
    ...
    const result = await query(
      `UPDATE connections 
       SET provider = CASE WHEN $1::text IS NULL THEN provider ELSE $1 END,
           api_url = CASE WHEN $2::text IS NULL THEN api_url ELSE $2 END,
           api_key = CASE WHEN $3::text IS NULL THEN api_key ELSE $3 END,
           instance_name = CASE WHEN $4::text IS NULL THEN instance_name ELSE $4 END,
           instance_id = CASE WHEN $5::text IS NULL THEN instance_id ELSE $5 END,
           wapi_token = CASE WHEN $6::text IS NULL THEN wapi_token ELSE $6 END,
           name = CASE WHEN $7::text IS NULL THEN name ELSE $7 END,
           status = CASE WHEN $8::text IS NULL THEN status ELSE $8 END,
           show_groups = CASE WHEN $9::boolean IS NULL THEN show_groups ELSE $9 END,
           meta_token = CASE WHEN $10::text IS NULL THEN meta_token ELSE $10 END,
           meta_phone_number_id = CASE WHEN $11::text IS NULL THEN meta_phone_number_id ELSE $11 END,
           meta_waba_id = CASE WHEN $12::text IS NULL THEN meta_waba_id ELSE $12 END,
           away_message_enabled = CASE WHEN $13::boolean IS NULL THEN away_message_enabled ELSE $13 END,
           away_message = CASE WHEN $14::text IS NULL THEN away_message ELSE $14 END,
           out_of_office_message_enabled = CASE WHEN $15::boolean IS NULL THEN out_of_office_message_enabled ELSE $15 END,
           out_of_office_message = CASE WHEN $16::text IS NULL THEN out_of_office_message ELSE $16 END,
           business_hours = CASE WHEN $17::jsonb IS NULL THEN business_hours ELSE $17 END,
           updated_at = NOW()
       WHERE ${whereClause}
        RETURNING *`,
      params
    ...
    if (from && from !== 'undefined') {
      ...
      for (const cand of candidates) {
        ...
        if (existingId && existingId !== cand.id) {
          ...
          try {
            await query(`DELETE FROM conversation_tag_links WHERE conversation_id = $1`, [cand.id]);
            await query(`DELETE FROM conversation_notes WHERE conversation_id = $1`, [cand.id]);
