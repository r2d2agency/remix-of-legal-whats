...
router.patch('/:id', async (req, res) => {
  try {
    ...
    const result = await query(
      `UPDATE connections 
       SET provider = CASE WHEN $1::text IS NULL AND $1 IS NULL THEN provider ELSE $1 END,
           api_url = CASE WHEN $2::text IS NULL AND $2 IS NULL THEN api_url ELSE $2 END,
           api_key = CASE WHEN $3::text IS NULL AND $3 IS NULL THEN api_key ELSE $3 END,
           instance_name = CASE WHEN $4::text IS NULL AND $4 IS NULL THEN instance_name ELSE $4 END,
           instance_id = CASE WHEN $5::text IS NULL AND $5 IS NULL THEN instance_id ELSE $5 END,
           wapi_token = CASE WHEN $6::text IS NULL AND $6 IS NULL THEN wapi_token ELSE $6 END,
           name = CASE WHEN $7::text IS NULL AND $7 IS NULL THEN name ELSE $7 END,
           status = CASE WHEN $8::text IS NULL AND $8 IS NULL THEN status ELSE $8 END,
           show_groups = CASE WHEN $9::boolean IS NULL AND $9 IS NULL THEN show_groups ELSE $9 END,
