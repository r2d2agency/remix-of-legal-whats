...
- Use português brasileiro`;
  ...
  try {
    ...
    if (provider === 'openai') {
      ...
      response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${['gemini-1.0-pro', 'gemini-pro', 'gemini-1.5-flash', 'gemini-1.5-pro', 'gemini-2.0-flash'].includes(model) ? 'gemini-2.5-flash' : (model || 'gemini-2.5-flash')}:generateContent?key=${apiKey}`, {
        method: 'POST',
